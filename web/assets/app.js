...
export function waSeguimiento(s, puntos) { return `Hola, ${s.nombre_completo.split(' ')[0]}. Sobre tu solicitud de afiliación al SITAD, folio *${s.folio}*: revisamos lo que subiste y todavía hace falta corregir lo siguiente:\n\n${puntos.map((f, i) => (i + 1) + '. ' + f).join('\n')}\n\nSúbelo corregido en esta liga (entra con tu folio y tu nombre o correo): ${ligaTramite(s)}${s.estado === 'Prevenida' && s.limite_subsanacion ? '\n\nRecuerda que la fecha límite es el *' + fechaLarga(s.limite_subsanacion) + '*.' : ''}\n\nSi tienes dudas, responde por aquí o acude a tu Sección: te ayudan a subirlo.\n\nSecretaría de Organización · CEN · SITAD`; }
export function correoSeguimiento(s, puntos) { return { asunto: `SITAD · Seguimiento a tu solicitud · Folio ${s.folio}`, cuerpo: waSeguimiento(s, puntos).replace(/\*/g, '') }; }
