-- 005 · Vistas: bandeja de solicitudes con semáforo, padrón por sección, tablero de indicadores (SIO hoja TABLERO).
...
create or replace view v_padron with (security_invoker = true) as
select p.*, sec.denominacion as seccion, sec.numero as seccion_numero, sec.slug as seccion_slug
from padron p join secciones sec on sec.id = p.seccion_id;

create or replace view v_secciones with (security_invoker = true) as
select s.*,
...
-- Tablero: una fila por indicador (bloques 1-6 del SIO). Se lee con: select * from v_tablero order by bloque, orden;
